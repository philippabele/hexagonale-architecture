...
public class ArchitectureTest_Task_4_1 extends BaseArchUnitExerciseTest {
    ...
    @Test
    @DisplayName("EquipmentCode as value object does not depend on other classes")
    void equipment_code_value_object_check() {
        ArchRule rule = classes()
                .that()
                .haveSimpleName("EquipmentCode")
                .and()
                .resideInAnyPackage(VEHICLE_DOMAIN_MODEL, VEHICLE_DOMAIN)
                .should()
                .onlyAccessClassesThat()
                .resideInAnyPackage(IO_GITHUB_DOMAINPRIMITIVES, JAVA_LANG, JAVA_UTIL)
                .orShould().haveSimpleName("EquipmentCode");
        rule.check(classes);
    }
